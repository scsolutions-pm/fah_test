...
<tbody>
<?php 
if( !empty($booking_data) && $booking_data->num_rows() > 0){
...
<tr>
    ...
    echo $ci->Common_model->get_data('cab_request',array('booking_id'=>$row->booking_id))->num_rows();   
    ?>
    </td>
    <td>
     <?php 
    $ci =&get_instance();
    $ci->load->model('Common_model');
    echo $ci->Common_model->get_data('hotel_request',array('booking_id'=>$row->booking_id))->num_rows();   
    ?>
    </td>
    <td>
     <?php 
    $ci =&get_instance();
    $ci->load->model('Common_model');
    echo $ci->Common_model->get_data('flight_request',array('booking_id'=>$row->booking_id))->num_rows();   
    ?>
    </td>
    <td><font color="green"><?php echo $row->priority_name; ?></font></td>

    <?php if($user_type==2 || $user_type==3) { ?>
    <td>
        <a href="<?php  echo site_url('User_TD/pendingRqstDetailFrm/'.$row->booking_id.'/'.$act); ?>">
    ...
    if($user_type==1) { ?>
    <td>
        <a href="<?php  echo site_url('User_TD/pendingRqstDetailFrm/'.$row->booking_id); ?>">
        <button class="btn btn-alt btn-hover btn-yellow" >
        <span>Proceed</span>
        <i class="glyph-icon icon-arrow-right"></i>
        </button>
