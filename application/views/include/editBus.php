...
if( !empty($bus_data) && $bus_data->num_rows() > 0){
	
foreach($bus_data->result() as $row)
{ 
 ?>
<tr>
   <td>
     <span class="label bg-gradient-3">
    ...
    <td><?php echo $row->to_state_id; ?></td>
    <td><?php echo $row->journey_date; ?></td>
    <td><?php echo getBusType($row->bus_type); ?></td>
    <td><a href="<?php echo site_url('User/editForm/'.$row->bus_rqst_id.'/3'); ?>" class="btn btn-xs btn-info" title="Edit Request">E <i class="glyph-icon icon-remove"></i></a>
    
   <?php if(isset($row->c_status_id) && $row->c_status_id==1){ ?>
   	 <a href="<?php echo site_url('User/generateModalCdata/'.$row->bus_rqst_id); ?>" class="btn btn-xs btn-danger" title="Delete Request">D <i class="glyph-icon icon-trash-o"></i></a>
   
   <?php }else{ ?> 
   <a href="<?php echo site_url('User/cancelRqstDialogNew/1/'.$row->b_request_id); ?>" class="btn btn-xs btn-warning" title="Cancel Request"  data-toggle="modal" data-target="#cancelModal">C <i class="glyph-icon icon-remove"></i></a>
   <?php } ?> 
...
<script type="text/javascript">
    ...
    {
        $("a[data-target=#b_rqst_Modal]").click(function(ev) {
        ev.preventDefault();
        var target = $(this).attr("href");
        // load the url and show modal on success
        $("#b_rqst_Modal .modal-content").load(target, function() { 
        $("#b_rqst_Modal").modal("show"); 
        });
        });
    }
