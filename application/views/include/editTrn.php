...
if( !empty($train_data) && $train_data->num_rows() > 0){
	
foreach($train_data->result() as $row)
{ ?>
<tr>
   <td>
     <span class="label bg-gradient-8">
    <a href="<?php echo site_url('User/generateModalTdata/'.$row->t_request_id); ?>" data-toggle="modal" data-target="#h_rqst_Modal">
    ...
    <td><?php echo getTrainClass($row->train_class); ?></td>
    <td><a href="<?php echo site_url('User/editForm/'.$row->train_rqst_id.'/4'); ?>" class="btn btn-xs btn-yellow" title=""> &nbsp;&nbsp;Edit&nbsp;&nbsp; </a><br><br>
    
     
    <a href="<?php echo site_url('User/generateModalCdata/'.$row->t_request_id); ?>" class="btn btn-xs btn-danger" title="">Delete</a>
    </td>
</tr>
<?php }  }?>
</tbody>
