...
<tr>
    ...
    <td><?php echo getcabType($row->cab_type); ?></td>
    <td><?php echo getcabPackage($row->cab_package); ?></td>
    <td><a href="<?php echo site_url('User/editForm/'.$row->cab_rqst_id.'/1'); ?>" class="btn btn-xs btn-info" title="Edit Request">E <i class="glyph-icon icon-remove"></i></a>
    
   <?php if(isset($row->c_status_id) && $row->c_status_id==1){ ?>
   	 <a href="<?php echo site_url('User/generateModalCdata/'.$row->c_request_id); ?>" class="btn btn-xs btn-danger" title="Delete Request">D <i class="glyph-icon icon-trash-o"></i></a>
   
   <?php }else{ ?> 
   <a href="<?php echo site_url('User/cancelRqstDialogNew/1/'.$row->c_request_id); ?>" class="btn btn-xs btn-warning" title="Cancel Request"  data-toggle="modal" data-target="#cancelModal">C <i class="glyph-icon icon-remove"></i></a>
   <?php } ?> 
