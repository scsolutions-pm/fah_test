...
function openChatBox(fname,lname,receiver_id,sender_id){
		...
		$( '#mycontent').append('<div class="container col-md-4 mydivss" id="chat_window_'+totalChatbox+'" style="z-index:9999"><div  id="added_'+receiver_id+'" class="row chat-window col-xs-5 col-md-3"  style="margin-left:10px;"><div class="col-xs-12 col-md-12"><div class="panel panel-default"><div class="panel-heading top-bar"><div class="col-md-8 col-xs-8"><h3 class="panel-title"><span class="glyphicon glyphicon-comment"></span>Chat - '+fname+' '+lname+'</h3></div><div class="col-md-4 col-xs-4" style="text-align: right;"><a href="#"><span id="minim_chat_window" class="glyphicon glyphicon-minus icon_minim"></span></a><a href="#"><span class="glyphicon glyphicon-remove icon_close" data-id="chat_window_1" onClick="closeChatBox('+totalChatbox+')"></span></a></div></div><div class="panel-body msg_container_base" id="mychat_'+totalChatbox+'"></div><div class="panel-footer"><div class="input-group"><input type="hidden" name="receiver_id" id="receiver_id_'+totalChatbox+'" value="'+receiver_id+'"><input type="hidden" name="sender_id" id="sender_id_'+totalChatbox+'" value="'+sender_id+'"><input id="messages_'+totalChatbox+'" type="text" class="form-control input-sm chat_input" placeholder="Write your message here..." /><span class="input-group-btn"><button class="btn btn-primary btn-sm" id="btn-chat" onClick="sendmsg('+totalChatbox+')">Send</button></span></div></div></div></div></div></div>');
		   
			 $.ajax({
        url: '<?php echo site_url();?>/User/getChat',
        type: 'POST',    
        data: { receiver_ids : receiver_id , sender_ids : sender_id},

        success: function (data) {
			
				$('#mychat_'+totalChatbox+'').html(data);		
			},
        error: function () {
                              alert('error');
                            }
...
function sendmsg(srt){
    var receiver_id =  $("#receiver_id_"+srt+"").val();
	var sender_id   =  $("#sender_id_"+srt+"").val();
  	var messages    =  $("#messages_"+srt+"").val();
	$.ajax({
        url: '<?php echo site_url();?>/User/sendMessages',
        type: 'POST',    
        data: { receiver_ids : receiver_id , sender_ids : sender_id, messagess : messages},

        success: function (data) {
			$('#mychat_'+srt+'').html(data);	
			$("#messages_"+srt+"").val("");	
			$('.panel-body.msg_container_base').animate({scrollTop: $(".panel-body.msg_container_base").prop("scrollHeight")}, 1000);
			//$(".panel-body.msg_container_base").prop({ scrollTop: $(".panel-body.msg_container_base").prop("scrollHeight") });
			},
        error: function () {
                              alert('error');
                            }
