...
<tbody>
<?php 
if( !empty($hotel_data) && $hotel_data->num_rows() > 0){
foreach($hotel_data->result() as $row)
{ ?>
<tr>
    <td><span class="label bg-gradient-3">
    <a href="<?php echo site_url('User/generateModalCdata/'.$row->h_request_id); ?>" data-toggle="modal" data-target="#h_rqst_Modal">    
    ...
    <td>
     ...
     <a href="<?php echo site_url('Tdesk/cancelRqstDialog/2/'.$row->h_request_id); ?>" class="btn btn-xs btn-danger" title="Cancel Request"  data-toggle="modal" data-target="#cancelModal">C <i class="glyph-icon icon-remove"></i></a>
    </td> 

    <?php if($flg =='P') { ?>
    <td class="actions">
       <a href="<?php echo site_url(); ?>/Tdesk/htlVdoForm/<?php echo $row->h_request_id; ?>" data-toggle="modal" data-target="#VdoModal"><button class="btn btn-alt btn-hover btn-yellow" >
        <span>Execute</span>
